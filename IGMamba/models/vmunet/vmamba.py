...
class Channel_Att_Bridge(nn.Module):
    def __init__(self, c_list=[96, 192, 384, 768, 1536], split_att='fc'):
        super().__init__()
...
class SC_Att_Bridge(nn.Module):
    ...
    def forward(self, t1, t2, t3, t4, t5):
        ...
        t3 = t3.permute(0, 3, 1, 2)
        t4 = t4.permute(0, 3, 1, 2)
        t5 = t5.permute(0, 3, 1, 2)
        ...
        x1 = (t1 + r1_).permute(0, 2, 3, 1)
        x2 = (t2 + r2_).permute(0, 2, 3, 1)
        x3 = (t3 + r3_).permute(0, 2, 3, 1)
        x4 = (t4 + r4_).permute(0, 2, 3, 1)
        x5 = (t5 + r5_).permute(0, 2, 3, 1)

        skip_list = [x1, x2, x3, x4, x5]
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        # self.cam = ChannelAttention(768)

        # factory_kwargs = {"device": device, "dtype": dtype}

        # self.in_proj = nn.Linear(772, 768, bias=bias, **factory_kwargs)
    ...
    # def forward_bridge(self, skip_list):
    #     t1 = skip_list[0]
    #     t2 = skip_list[1]
    #     t3 = skip_list[2]
    #     t4 = skip_list[3]
    #     t5 = skip_list[4]
    #     skip_list = self.scab(t1, t2, t3, t4, t5)
    #     return skip_list
    #
    ...
    def forward(self, x1):
        ...
        # x2 = F.relu(self.rn(x))
        # x1 = self.spp(x1)
        # attention_value = self.cam(x1)# 10,2212,16,16
        # x1 = x1.mul(attention_value)
        # x1 = x1.permute(0, 2, 3, 1).contiguous()
        # skip_list = self.forward_bridge(skip_list)
        # x1 = self.in_proj(x1)
